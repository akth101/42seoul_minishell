...
#### 2-2) 프로세스 대기 문제로 인한 병렬 실행 불가능 상황 해결하기
미니쉘 구현 중 파이프로 연결된 명령어를 처리할 때 심각한 문제를 발견했습니다. 부모 프로세스에서 자식 프로세스를 생성한 직후 parent_to_do 함수에서 wait을 호출하여 자식 프로세스가 종료될 때까지 부모 프로세스가 블록되는 상황이었습니다.
~~~ c
...
이로 인해 파이프로 연결된 여러 명령어(예: cmd1 | cmd2 | cmd3)를 실행할 때, 첫 번째 자식 프로세스(cmd1)가 종료되기 전까지 다음 프로세스들을 생성하지 못하는 병목 현상이 발생했습니다. 이는 파이프의 핵심 기능인 병렬 처리를 불가능하게 만들었는데 뭔가 이상했습니다. minishell 과제를 하기 전에 멀티 프로세싱을 연습하는 pipex라는 과제를 수행했음에도 불구하고 이런 기본적인 문제가 터진다는 것은 아직 제가 이 과정을 완전히 장악하고 있지 못하다는 것을 뜻했습니다. 그래서 다시 멀티 프로세싱 개념과 pipex 코드를 뜯어보며 어디에 구멍이 난 것인지 면밀히 살펴본 결과 wait 메서드의 블로킹이라는 동작을 정확하게 파악하지 않은 채로 넘어갔다는 결론을 내렸습니다. 원인 파악이 끝난 후 별도의 wait_child 함수를 구현하고 메인 로직을 수정하여 모든 자식 프로세스를 먼저 생성한 후 마지막에 대기하도록 하여 문제를 해결했습니다:
~~~ c
while (++i < process.pipe_cnt + 1)
{
    // 파이프 및 프로세스 생성
...
만약 멀티 프로세스를 연습하는 프로젝트인 pipex에서 wait 메서드의 정확한 동작을 알고 넘어갔다면 발생하지 않았을 문제였을 것입니다. 이 문제해결 과정을 통해 제가 알고 있다고 생각하는 개념도 다시 한 번 묻고 의심하는 습관을 갖게 되는 계기가 되었습니다. 
